...
(protocol DECISIONS D10). Don't reintroduce it unless the operator asks. Commits and pushes are still
validated for every tool through the git hooks (`config/git-hooks`, installed by `npm prepare` via
`core.hooksPath`).
