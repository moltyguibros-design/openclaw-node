...
const TOOLS = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        path_prefix: {
          type: 'string',
          description: 'Optional directory prefix (relative to the knowledge root, e.g. "memory/" or "projects/arcane/lore") — only documents under it are returned. Use knowledge_tree to discover prefixes.',
        },
      },
      required: ['query'],
    },
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        path_prefix: {
          type: 'string',
          description: 'Optional directory prefix — only related documents under it are returned.',
        },
      },
      required: ['doc_path'],
    },
  ...
  },
  {
    name: 'knowledge_tree',
    description:
      'Browse the knowledge base as a directory tree. Every directory carries a one-line abstract (L0) so you can judge relevance before searching or reading. Call this first to pick a path_prefix for semantic_search, or a path for directory_overview.',
    annotations: { readOnlyHint: true },
    inputSchema: {
      type: 'object',
      properties: {
        path_prefix: { type: 'string', description: 'Directory to start from (default: root)' },
        depth: { type: 'number', description: 'Levels below the start directory (default 2)', default: 2 },
      },
    },
  },
  {
    ...
    description:
      'The L1 overview of one directory: its subdirectories with their abstracts and its documents with their titles. Read this before opening individual files.',
    annotations: { readOnlyHint: true },
    inputSchema: {
      type: 'object',
      properties: {
        path: { type: 'string', description: 'Directory path relative to the knowledge root ("" for root)' },
      },
      required: ['path'],
    },
  },
  {
    ...
    description:
      'Rank directories by how well their abstract matches a query — find the right folder before the right file. Follow up with semantic_search using the returned dir_path as path_prefix.',
    annotations: { readOnlyHint: true },
    inputSchema: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'Natural language query' },
        limit: { type: 'number', description: 'Max directories (default 5)', default: 5 },
      },
      required: ['query'],
    },
  },
    ...
    try {
      switch (name) {
        case 'semantic_search': {
          const results = await engine.search(args.query, args.limit || 10, { pathPrefix: args.path_prefix });
          return { content: [{ type: 'text', text: JSON.stringify(results, null, 2) }] };
        }
        case 'find_related': {
          const results = await engine.related(args.doc_path, args.limit || 10, { pathPrefix: args.path_prefix });
          return { content: [{ type: 'text', text: JSON.stringify(results, null, 2) }] };
        }
        case 'reindex': {
          if (!allowReindex) {
        ...
        case 'knowledge_tree': {
          const rows = engine.tree(args?.path_prefix || '', args?.depth ?? 2);
          return { content: [{ type: 'text', text: JSON.stringify(rows, null, 2) }] };
        }
        case 'directory_overview': {
          const row = engine.directory(args.path ?? '');
          if (!row) return { content: [{ type: 'text', text: `No summary for directory: ${args.path}` }], isError: true };
          return { content: [{ type: 'text', text: JSON.stringify(row, null, 2) }] };
        }
        case 'search_directories': {
          const rows = await engine.searchDirectories(args.query, args.limit || 5);
          return { content: [{ type: 'text', text: JSON.stringify(rows, null, 2) }] };
        }
        case 'sql_query': {
...
// Directory abstracts are LLM-written when the local model answers, else
// deterministic. KNOWLEDGE_SUMMARY_LLM=0 keeps the index pass model-free.
async function loadSummaryLlmClient() {
  if (process.env.KNOWLEDGE_SUMMARY_LLM === '0') return null;
  try {
    const { createLlmClient } = await import('../llm-client.mjs');
    const client = createLlmClient();
    const health = await client.healthCheck();
    if (!health.ok) {
      process.stderr.write(`[mcp-knowledge] llm not reachable (${health.error}); directory summaries deterministic\n`);
      return null;
    }
    return client;
  } catch (err) {
    process.stderr.write(`[mcp-knowledge] llm client unavailable (${err.message}); directory summaries deterministic\n`);
    return null;
  }
}

async function main() {
  const engine = await createKnowledgeEngine({ llmClient: await loadSummaryLlmClient() });
