...
export function verifySignedRequest(event, opts = {}) {
  ...
  // The signer is returned, not just checked: it is the one identity in a
  // request that is cryptographically established (node_id in the envelope is
  // self-asserted). Callers that record WHO did something need this — without
  // it they can only invent a value.
  return { ok: true, reason: 'verified', signer_pubkey: event.signer_pubkey };
}
