...
/**
 * Worker-reported states that mean "alive but not progressing" — the agent is
 * waiting on a human or has hit an error. Reported by lib/agent-activity.js.
 */
const STUCK_ACTIVITY_STATES = ['waiting_input', 'blocked'];
...
class TaskStore {
  constructor(kv, { leaseMs = DEFAULT_LEASE_MS } = {}) {
    ...
    tracer.wrapClass(this, [
      ...
      'markReleased', 'touchActivity', 'markStallCleared', 'findStalled', 'findNotProgressing', 'findOverBudget',
   ...
   */
  /**
   * A heartbeat renews the lease and resets the stall clock, so a worker that
   * is stuck but still alive used to look identical to one making progress.
   * The reported state is recorded here so the enforcement loop can tell the
   * difference; `activity_state_since` marks when the CURRENT state began.
   */
  async touchActivity(taskId, { activityState = null, activityTimestamp = null } = {}) {
    return this._updateWithCAS(taskId, (task) => {
      if (TERMINAL_STATES.has(task.status)) return null;
      const now = new Date().toISOString();
      task.last_activity = now;
      if (activityState) {
        if (task.activity_state !== activityState) task.activity_state_since = now;
        task.activity_state = activityState;
        task.activity_timestamp = activityTimestamp || now;
      }
      // A heartbeat the worker sent itself is the signal the clear-window was
      // standing in for, so it ends the run of daemon-side clears.
      delete task.stall_cleared_since;
      delete task.stall_clear_count;
      this._renewLease(task);
      return task;
    });
  }
  ...
  /**
   * The daemon asked a stalled task's agent whether it is alive and got yes.
   * That extends the deadline, but consecutive clears are counted and dated:
   * an agent can answer "alive" truthfully while making no progress at all,
   * and without a bound it resets the stall detector forever.
   */
  async markStallCleared(taskId) {
    return this._updateWithCAS(taskId, (task) => {
      if (TERMINAL_STATES.has(task.status)) return null;
      const now = new Date().toISOString();
      task.last_activity = now;
      if (!task.stall_cleared_since) task.stall_cleared_since = now;
      task.stall_clear_count = (task.stall_clear_count || 0) + 1;
      this._renewLease(task);
      return task;
    });
  }

  /** Minutes since the current run of alive-check clears began, or null. */
  static stallClearedForMinutes(task, now = Date.now()) {
    if (!task?.stall_cleared_since) return null;
    return (now - new Date(task.stall_cleared_since).getTime()) / 60000;
  }

  /**
   * Tasks whose worker has reported a non-progressing state for longer than
   * `minutes`. `waiting_input` is an agent parked on a permission prompt and
   * `blocked` is one that hit an error: both keep heartbeating, so neither is
   * caught by findStalled.
   */
  async findNotProgressing(minutes = 10, states = STUCK_ACTIVITY_STATES, { heartbeatWithinMs = null } = {}) {
    ...
    return [...running, ...claimed].filter((t) => {
      if (!states.includes(t.activity_state)) return false;
      // A worker that died while parked keeps its last reported state forever.
      // Releasing that as "needs a human" is the wrong label and skips the
      // collab death-marking the stall path does, so leave it to findStalled.
      if (heartbeatWithinMs !== null) {
        const beat = t.last_activity || t.started_at;
        if (!beat || now - new Date(beat).getTime() > heartbeatWithinMs) return false;
      }
      const since = t.activity_state_since || t.last_activity || t.started_at;
      return since && new Date(since).getTime() < cutoff;
    });
  }
...
module.exports = {
  STUCK_ACTIVITY_STATES, createTask, TaskStore, TASK_STATUS, TERMINAL_STATES, DEFAULT_MAX_REJECTIONS, DEFAULT_LEASE_MS, DEFAULT_MAX_LEASE_REQUEUES, KV_BUCKET };
