...
class TaskStore {
   ...
   *
   * `reviewedBy` is the identity the caller ESTABLISHED (the daemon passes
   * authorizeTaskMutation's `actor`). It used to be the literal 'human', which
   * recorded the same thing whether a person or a cron approved the task.
   */
  async markApproved(taskId, reviewedBy) {
    return this._updateWithCAS(taskId, (task) => {
      if (task.status !== TASK_STATUS.PENDING_REVIEW) return null;
      task.status = TASK_STATUS.COMPLETED;
      task.completed_at = new Date().toISOString();
      task.reviewed_by = reviewedBy || 'unattributed';
      return task;
    });
  }
