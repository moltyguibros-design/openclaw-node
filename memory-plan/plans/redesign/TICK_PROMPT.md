...
3. **Phase 4 · implement** — only the §6 deltas. Mid-implementation surprises → AUDIT_PRE `## Mid-Implementation Findings`. **Tripwire (WORKFLOW §7.3): if this sprawls into many sub-actions or ≥2 mid-impl findings, the step wasn't atomic — STOP, write BLOCKED.md proposing a split.**
...
- Do NOT widen the step to chase an unrelated observation — record it under AUDIT_PRE `## Mid-Implementation Findings`.
