---
name: archify
description: "Renders validated architecture, workflow, sequence, data-flow and lifecycle diagrams from a small typed JSON spec into self-contained HTML with inline SVG. Use when a diagram must reflect real structure, or to convert Mermaid into something that validates."
triggers:
...
negative_triggers:
  - "ascii diagram"
  - "chart these numbers"
  - "design a figma mockup"
license: MIT
metadata: {"clawdbot":{"emoji":"\ud83d\uddfa\ufe0f","requires":{"bins":["node"]},"env":{"ARCHIFY_UPDATE_CHECK_DISABLED":"1"},"source":"tt-a1i/archify 2.17 (MIT), based on Cocoon-AI/architecture-diagram-generator (MIT)"}}
---
...
Trimmed for the node: the runtime (`bin/`, `renderers/`, `schemas/`, `assets/`) plus `delta/`,
`recipes/`, `migrations/`, `brand-marks/` and `references/`. Upstream's 3.6 MB of examples and its
test tree are not shipped, so `doctor` and `demo` will not run; `validate` and `deliver` are what
this skill is for. The update checker is not shipped either, and `ARCHIFY_UPDATE_CHECK_DISABLED=1`
is set in `openclaw.env.example` as a second layer — this skill makes no network request.

Run it from the skill directory, e.g. `cd skills/archify && node bin/archify.mjs validate lifecycle
examples/lifecycle.example.json --quality showcase --json`.
